...
const EditSidebar: React.FC<EditSidebarProps> = ({ isOpen, onGoHome, onClose }) => {
    ...
    const handleSaveChanges = useCallback(async () => {
        if (!analysisResult) {
            setError("There is no book to analyze currently.");
            return;
        }
        setIsLoading(true);
        ...
        try {
            ...
            alert(`Changes to "${bookToSave.name}" were saved successfully.`);
        } catch (err: any) {
                    ...
                    <div className="flex-grow flex flex-row-reverse min-h-0">
                        ...
                        <>
                            ...
                            <nav className="w-1/4 border-r border-[var(--color-border-primary)] flex-shrink-0 flex flex-col no-print-sidebar">
                                ...
                                ) : ( <div className="text-center p-4 text-gold-brown text-sm"> {file ? 'Click "Analyze" to start.' : 'Select a PDF file or load a book from your library.'} </div> )}
                                </div>
                                ...
                                {selectedPage ? (
                                    ...
                                    <div className="flex items-center justify-center h-full">
                                        <p className="text-center text-lg text-gold-brown">
                                            {analysisResult ? 'Select a page from the list to view and edit it.' : (file ? 'Click "Analyze" to start.' : 'Select a PDF file or load a book from your library.')}
                                        </p>
                                    </div>
                                )}
                            </div>
