...
import { XIcon, UploadIcon, HomeIcon, PdfIcon, SearchIcon } from './icons';
            ...
            case 'upload':
                return (
                    <div className="p-6 flex flex-col items-center justify-center h-full text-center">
                        ...
                        <h3 className="text-xl font-bold mb-2 golden-text">البحث داخل المستندات</h3>
                        <p className="text-sm text-gold-brown mb-6 max-w-sm">ارفع أي ملف وابحث في محتواه للعثور على معلومات دقيقة بسرعة.</p>
                        ...
                        <button onClick={handleAnalyze} disabled={!file} className="w-full max-w-xs px-8 py-3 text-lg font-bold text-white rounded-lg shadow-lg disabled:opacity-50" style={{ backgroundImage: goldenGradient }}>ابدأ البحث</button>
                    </div>
                );
            case 'search':
                return (
                    <div className="flex-grow flex flex-col min-h-0">
                         <div className="flex-shrink-0 p-4 border-b border-[var(--color-border-primary)]">
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full p-3 pl-10 rtl:pr-10 bg-[var(--color-background-tertiary)] rounded-lg border border-[var(--color-border-primary)] focus:ring-2 focus:ring-[var(--color-accent-primary)] transition"
                                />
                         ...
                         <div className="flex-grow flex flex-col md:flex-row min-h-0">
                            <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-l border-[var(--color-border-primary)] overflow-y-auto p-3">
                                {searchTerm ? (
                                    searchResults.length > 0 ? (
                                        <div className="space-y-2">
                                            {searchResults.map((result, index) => (
                                                <button key={index} onClick={() => handleResultClick(result.pageNumber)} className="w-full text-right p-2 rounded-md hover:bg-[var(--color-background-tertiary)]">
                                                    ...
                                                    <span className="text-xs font-bold text-dark-gold-gradient/70">الصفحة: {result.pageNumber}</span>
                                                </button>
                                            ))}
                                        </div>
                                    ) : <p className="text-center text-sm text-gold-brown p-4">لا توجد نتائج بحث.</p>
                                ) : (
                                    <p className="text-center text-sm text-gold-brown p-4">أدخل مصطلح بحث لعرض النتائج هنا.</p>
                                )}
                            </div>
                                ...
                                {selectedPageNumber ? (
                                    pdfDoc ? (
                                        ...
                                        <div className="p-4" style={{ fontFamily: "'Times New Roman', serif" }}>
                                            <h4 className="font-bold text-lg mb-2 text-dark-gold-gradient">محتوى المستند</h4>
                                    ...
                                    <div className="flex items-center justify-center h-full text-center text-gold-brown">
                                        <p>انقر على نتيجة بحث لعرض الصفحة هنا.</p>
                                    </div>
                                )}
                            </div>
                         </div>
            ...
            {isOpen && (
                <>
                    <div className="flex-shrink-0 p-4 border-b border-[var(--color-border-primary)] flex justify-between items-center bg-[var(--color-background-primary)] no-print-sidebar">
                        ...
                        <h2 className="text-xl font-bold golden-text flex items-center gap-2"><SearchIcon className="w-6 h-6 golden-text" /> الباحث الذكى</h2>
